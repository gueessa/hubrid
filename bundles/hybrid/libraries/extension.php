...
class Extension {
   	...
   	public static function start($name, $config = array())
    {
        ...
        if (is_file($file = Bundle::path($name).'hybrid/hybrid.php'))
        {
            include_once $file;
        }
    ...
    public static function detect()
    {
        ...
        if (is_file(path('app').'hybrid/hybrid.json'))
        {
            $extensions[DEFAULT_BUNDLE] = json_decode(file_get_contents(path('app').'hybrid/hybrid.json'));
        }
        ...
        foreach ($items as $item)
        {
            if ($item->isDir())
            {
                if (is_file($item->getRealPath().'hybrid/hybrid.json'))
                {
                    $extensions[$item->getFilename()] = json_decode(file_get_contents($item->getRealPath().'hybrid/hybrid.json'));
                }
            }
        }
    ...
    public static function activate($name)
    {
        ...
        if (isset($available[$name]))
        {
            ...
            {
                IoC::resolve('task: hybrid.migrator', array('migrate', $name));
            }

            if (IoC::registered('task: hybrid.publisher'))
            {
                IoC::resolve('task: hybrid.publisher', array($name));
            }
        }
